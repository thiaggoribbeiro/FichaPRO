...
const PropertyDetails: React.FC<PropertyDetailsProps> = ({ property, allProperties, onBack, onGenerateFicha, onEditProperty, onDeleteProperty, showToast, user }) => {
    ...
    const aggregatedData = React.useMemo(() => {
        ...
        const sums = {
            ...
            matricula: units.map(u => u.registration).filter(Boolean).join(', '),
    ...
    const displayMatricula = isComplex ? aggregatedData?.matricula : property.registration;
    ...
    const detailCards = [
        ...
        { label: 'Área Construída', value: `${displayBuiltArea || 0} m²`, icon: <Square className="w-5 h-5" />, color: 'bg-blue-50 text-orange-600' },
